  ...
  <ion-page>
    <ion-content :fullscreen="true" class="ct">
      <div class="bar">
        ...
        <div class="ttl"><div class="s">Cierre del día</div><div class="n">Corte de caja</div></div>
        <div class="iconbtn"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg></div>
      </div>

      <div class="body" v-if="!cargando && resumen">
        <div class="hero">
          <div class="top"><span class="t">Ventas del día</span><span class="tag">{{ fechaTag }}</span></div>
          <div class="val">{{ money(resumen.totalVentas) }}<span class="mxn">MXN</span></div>
          <div class="sub">
            <div><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#7FD3BD" stroke-width="2.2" stroke-linecap="round"><path d="M20 6L9 17l-5-5"/></svg><b>{{ resumen.numeroPedidos }}</b>&nbsp;entregas</div>
            <div><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#7FD3BD" stroke-width="2.2" stroke-linecap="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/></svg><b>{{ resumen.numeroPedidos }}</b>&nbsp;paradas</div>
          </div>
        </div>
        ...
        <p v-if="resumen.yaTieneCorte" class="cerrado">Ya cerraste el corte de hoy.</p>
      ...
      <div class="foot" v-if="!cargando && resumen && !resumen.yaTieneCorte && !done">
        <button class="cta" :disabled="enviando" @click="cerrar()">
          {{ enviando ? 'Cerrando…' : 'Cerrar corte del día' }}
      ...
      <div class="done" :class="{ show: done }" v-if="corte">
        ...
        <div class="slip">
          <div class="h"><div class="b">CORTE DE CAJA</div><small>Repartidor: {{ corte.repartidorNombre }}</small></div>
...
const fechaTag = computed(() => resumen.value
  ? new Date(resumen.value.fecha).toLocaleDateString('es-MX', { weekday: 'short', day: '2-digit', month: 'short' })
  : '')
...
.hero .top .tag { font-size: 11px; font-weight: 700; color: #9FC9BC; background: rgba(255,255,255,.10); padding: 5px 10px; border-radius: 999px; text-transform: capitalize; }
