<template>
  <div>
    ...
    <template v-else>
      ...
      <p v-if="!conDeuda.length" class="muted">Ningún cliente tiene saldo pendiente.</p>

      <div class="grid" v-if="conDeuda.length">
        <div v-for="d in conDeuda" :key="d.clienteId" class="card">
          <div class="c-top">
            <div class="av">{{ ini(d.clienteNombre) }}</div>
            <div class="info">
              <div class="nm">{{ d.clienteNombre }}</div>
              <div class="meta">Debe {{ money(d.totalCargado) }} · Abonado {{ money(d.totalAbonado) }}</div>
            </div>
      ...
      <!-- Clientes sin deuda (ya liquidaron todo con abonos) -->
      <div v-if="sinDeuda.length" class="limpios">
        <div class="lm-t">Sin saldo pendiente</div>
        <div class="lm-list">
          <span v-for="d in sinDeuda" :key="d.clienteId" class="lm">{{ d.clienteNombre }}</span>
        </div>
      </div>
    </template>
      ...
      <div class="modal">
        <div class="m-head">
          <div>
            ...
            <div class="m-sub">{{ modal.clienteNombre }}</div>
          </div>
        ...
        <div class="m-body">
          ...
          <div class="campo">
            ...
            <input class="inp" v-model="nota" placeholder="Ej. Pago en efectivo">
          </div>
...
async function guardar() {
  ...
  try {
    await http.post('/creditos/abonos-cliente', {
      clienteId: modal.value.clienteId,
...
onMounted(() => {
  emit('ctx', { titulo: 'Deudas de clientes', sub: 'Ventas a crédito pendientes y abonos', back: null })
  cargar()
})
</script>
