  ...
  <div class="editor">
    ...
    <template v-else>
      <div class="form" v-show="!exito">
        <div class="field"><div class="fl">Nombre completo *</div><input class="inp" v-model="form.nombre" placeholder="Ej. Sofía Torres"></div>
        <div class="field"><div class="fl">Correo (para iniciar sesión) *</div><input class="inp" v-model="form.email" type="email" placeholder="ej. sofia@distribuidora.mx" autocapitalize="off"></div>
        <div class="field">
      ...
      <div class="guardar-bar" v-show="!exito">
        <button class="cta" :disabled="enviando || !valido" @click="guardar()">{{ enviando ? 'Guardando…' : (esNuevo ? 'Crear vendedor' : 'Guardar cambios') }}</button>
      </div>
    </template>
    <ExitoOverlay :show="exito" :titulo="exitoTit" :subtitulo="form.nombre" :detalle="exitoDet" cta-texto="Ver vendedores" @done="salir" />
  </div>
</template>
...
function salir() { router.replace('/panel/vendedores') }
async function guardar() {
  if (!valido.value) return
  enviando.value = true; error.value = ''
  try {
    if (esNuevo.value) {
      await http.post('/usuarios', { nombre: form.nombre.trim(), email: form.email.trim(), password: form.password }, { params: { rol: 'Vendedor' } })
    } else {
...
onMounted(async () => {
  emit('ctx', { titulo: esNuevo.value ? 'Nuevo vendedor' : 'Editar vendedor', sub: '', back: '/panel/vendedores' })
  if (!esNuevo.value) {
    ...
    catch { error.value = 'No se pudo cargar el vendedor.' }
  }
