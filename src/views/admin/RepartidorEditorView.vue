<template>
  <div class="editor">
    <p v-if="cargando" class="muted">Cargando repartidor…</p>
    <template v-else>
      <div class="form" v-show="!exito">
        <div class="field">
          <div class="fl">Nombre completo *</div>
          <input class="inp" v-model="form.nombre" placeholder="Ej. Luis Salazar" autocomplete="name">
        </div>

        <div class="field">
          <div class="fl">Correo electrónico (para iniciar sesión) *</div>
          <input class="inp" v-model="form.email" type="email" placeholder="ej. luis@distribuidora.mx" autocapitalize="off" autocomplete="username">
          <div class="field-hint">Con este correo iniciará sesión en la aplicación móvil de reparto.</div>
        </div>
        ...
        <div class="field toggle" v-if="!esNuevo">
          <div>
            ...
            <div class="td">Si desactivas esta cuenta, el repartidor no podrá abrir cargas ni registrar entregas en la app.</div>
          </div>
      ...
      <div class="guardar-bar" v-show="!exito">
        <button class="cta" :disabled="enviando || !valido" @click="guardar()">
          {{ enviando ? 'Guardando…' : (esNuevo ? 'Crear repartidor' : 'Guardar cambios') }}
        </button>
      </div>
    </template>
    ...
    <ExitoOverlay
      ...
      :detalle="exitoDet"
      cta-texto="Volver a repartidores"
      @done="salir"
    />
  </div>
</template>
...
function salir() {
  router.replace('/panel/repartidores')
}
...
async function guardar() {
  ...
  try {
    ...
    error.value = e.response?.data?.mensaje || 'No se pudo guardar el repartidor.'
  } finally {
...
onMounted(async () => {
  emit('ctx', {
    titulo: esNuevo.value ? 'Nuevo repartidor' : 'Editar repartidor',
    sub: esNuevo.value ? 'Registra un nuevo chofer para reparto en ruta' : 'Modifica datos de acceso o estatus',
    back: '/panel/repartidores'
  })
  ...
  if (!esNuevo.value) {
    try {
      ...
      error.value = 'No se pudo cargar la información del repartidor.'
    }
  }
